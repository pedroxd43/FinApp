...
export default function CategoryEditScreen() {
  ...
  const [type, setType] = useState<TransactionType>(
    editingCat?.type || (typeParam as TransactionType) || 'expense',
  );
  ...
  const handleDelete = () => {
    ...
    Alert.alert(
      'Eliminar categoría',
      txCount > 0
        ? `Hay ${txCount} transacción(es) asociadas. ¿Eliminar de todos modos?`
      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <View style={[styles.preview, { backgroundColor: color + '22' }]}>
          {(() => {
            ...
            return <IconComp size={32} color={color} strokeWidth={2} />;
          })()}
        ...
        <TextInput
          style={[styles.nameInput, { backgroundColor: colors.card, color: colors.textPrimary, borderColor: colors.border }]}
          ...
          {CATEGORY_ICONS.map((iconName) => {
            ...
            return (
              <TouchableOpacity
                ...
                style={[
                  styles.iconBox,
                  { backgroundColor: icon === iconName ? color + '22' : colors.card },
