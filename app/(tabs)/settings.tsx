...
import { Wallet, Download, Trash2, Info, Sun, Moon } from 'lucide-react-native';
...
export default function SettingsScreen() {
  ...
  const handleClearTransactions = () => {
    Alert.alert(
      ...
      'Se eliminarán todas tus transacciones. Esta acción no se puede deshacer.',
      [
  ...
  const handleResetCategories = () => {
    Alert.alert(
      ...
      'Se restaurarán las categorías por defecto. Las categorías personalizadas no se eliminarán.',
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          text: 'Restaurar',
          onPress: () => {
            ...
            Alert.alert('Listo', 'Categorías por defecto restauradas');
          },
        },
      ],
    );
    ...
    <View style={[styles.container, { backgroundColor: colors.bg }]}>
      ...
      <View style={styles.section}>
        ...
        <TouchableOpacity style={[styles.row, { backgroundColor: colors.card }]} onPress={handleExport}>
          ...
          <View style={styles.rowContent}>
            <Text style={[styles.rowTitle, { color: colors.textPrimary }]}>Exportar datos</Text>
            <Text style={[styles.rowSubtitle, { color: colors.textSecondary }]}>Comparte tus transacciones y categorías</Text>
          </View>
        </TouchableOpacity>
        ...
        <TouchableOpacity style={[styles.row, { backgroundColor: colors.card }]} onPress={handleResetCategories}>
          <View style={[styles.rowIcon, { backgroundColor: colors.warningLight }]}>
            <Wallet size={20} color={colors.warningDark} />
          </View>
          <View style={styles.rowContent}>
            <Text style={[styles.rowTitle, { color: colors.textPrimary }]}>Restaurar categorías</Text>
            <Text style={[styles.rowSubtitle, { color: colors.textSecondary }]}>Volver a las categorías por defecto</Text>
          </View>
        </TouchableOpacity>
        ...
        <TouchableOpacity style={[styles.row, { backgroundColor: colors.card }]} onPress={handleClearTransactions}>
          ...
          <View style={styles.rowContent}>
            <Text style={[styles.rowTitle, { color: colors.textPrimary }]}>Borrar transacciones</Text>
            <Text style={[styles.rowSubtitle, { color: colors.textSecondary }]}>Eliminar todas las transacciones</Text>
          </View>
        </TouchableOpacity>
      </View>
      ...
      <View style={styles.section}>
        ...
        <View style={[styles.row, { backgroundColor: colors.card }]}>
          <View style={[styles.rowIcon, { backgroundColor: colors.incomeLight }]}>
            <Info size={20} color={colors.incomeDark} />
          </View>
          <View style={styles.rowContent}>
            <Text style={[styles.rowTitle, { color: colors.textPrimary }]}>FinApp</Text>
            <Text style={[styles.rowSubtitle, { color: colors.textSecondary }]}>Versión 1.0.0</Text>
          </View>
        </View>

        <View style={[styles.row, { backgroundColor: colors.card }]}>
          <View style={[styles.rowIcon, { backgroundColor: colors.expenseLight }]}>
            <Wallet size={20} color={colors.expenseDark} />
          </View>
          <View style={styles.rowContent}>
            <Text style={[styles.rowTitle, { color: colors.textPrimary }]}>{transactions.length} transacciones</Text>
            <Text style={[styles.rowSubtitle, { color: colors.textSecondary }]}>{categories.length} categorías registradas</Text>
          </View>
        </View>
      </View>

      <View style={styles.footer}>
        <Text style={[styles.footerText, { color: colors.textTertiary }]}>Tus datos se guardan localmente en tu dispositivo</Text>
      </View>
    </View>
  );
}
